...
class Utils {
    ...
    /**@description 显示视图动画 */
    showView(node, complete) {
        if (node) {
            cc_1.Tween.stopAllByTag(VIEW_ACTION_TAG);
            (0, cc_1.tween)(node).tag(VIEW_ACTION_TAG)
     ...
     * @returns
     */
    limitString(label, content, width = 100, suffix = "..") {
        if (label) {
            //计算内容大小
            label.string = content;
            label.forceDoLayout();
            let trans = label.getComponent(cc_1.UITransform);
            ...
            if (label.assemblerData && label.assemblerData.context) {
                let safeWidth = (0, cc_1.safeMeasureText)(label.assemblerData.context, subString);
                while (safeWidth > width) {
                    subString = subString.substring(0, subString.length - suffix.length - 1) + suffix;
                    safeWidth = (0, cc_1.safeMeasureText)(label.assemblerData.context, subString);
                }
            }
    ...
    toThousandths(data) {
        ...
        let str = data.toString().replace(digitParten, (m) => {
